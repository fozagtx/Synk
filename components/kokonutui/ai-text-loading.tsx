...
export default function AITextLoading({
  ...
  className,
  interval,
}: AITextLoadingProps): ReactElement {
  const [currentTextIndex, setCurrentTextIndex] = useState(0);
  const resolvedInterval: number = interval ?? 1500;
  const resolvedTexts: string[] =
    texts && texts.length > 0
      ? texts
      : ["Thinking...", "Processing...", "Analyzing...", "Computing...", "Almost..."];

  useEffect(() => {
    const timer = setInterval(() => {
      setCurrentTextIndex((prevIndex) => {
        return (prevIndex + 1) % resolvedTexts.length;
      });
    }, resolvedInterval);

    return () => clearInterval(timer);
  }, [resolvedInterval, resolvedTexts.length]);
    ...
    <div className="flex items-center justify-center p-8">
      ...
      >
        <AnimatePresence mode="wait">
          ...
          >
            {resolvedTexts[currentTextIndex]}
          </motion.div>
        </AnimatePresence>
      </motion.div>
    </div>
