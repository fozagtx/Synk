...
export const seoCheck: Check = {
  ...
  async run(ctx) {
    const findings = [];
    const seoHtml = ctx.renderedHtml ?? ctx.html;
    const title = pageTitle({ html: seoHtml });
    const description = metaContent({ html: seoHtml, name: "description" });
    const ogTitle = metaContent({ html: seoHtml, name: "og:title" });
    const canonical = canonicalUrl({ html: seoHtml });
    if (!title || title.length < 10 || title.length > 65) findings.push(finding({ id: "seo-title", checkId: "chk-02-seo", category: "seo", severity: "high", title: "The page title is missing or unclear", summary: "The browser title is empty or outside a useful search length.", whyItMatters: "Searchers cannot tell what the product does from the result page.", exactFix: "Add a clear 10 to 65 character title describing the product and its audience.", evidence: [requestEvidence({ request: `GET ${ctx.resolvedUrl}`, status: ctx.status })] }));
    if (!description || description.length < 50) findings.push(finding({ id: "seo-description", checkId: "chk-02-seo", category: "seo", severity: "medium", title: "The search description is missing or too short", summary: "There is no useful meta description for search previews.", whyItMatters: "Search engines and social previews have less context to show potential customers.", exactFix: "Add a 50 to 160 character meta description that says who the product helps and how.", evidence: [requestEvidence({ request: `GET ${ctx.resolvedUrl}`, status: ctx.status })] }));
    ...
    if (hasNoIndex({ html: seoHtml })) findings.push(finding({ id: "seo-noindex", checkId: "chk-02-seo", category: "seo", severity: "blocker", title: "The site tells search engines not to index it", summary: "A noindex robots directive is present.", whyItMatters: "Your public product may never appear in search results.", exactFix: "Remove noindex from production pages unless the page is intentionally private.", evidence: [requestEvidence({ request: `GET ${ctx.resolvedUrl}`, status: ctx.status })] }));
    if (hasEmptyRoot({ html: ctx.html })) {
      const renderedContent = ctx.renderedHtml !== null && !hasEmptyRoot({ html: ctx.renderedHtml });
      findings.push(finding({ id: "seo-empty-root", checkId: "chk-02-seo", category: "seo", severity: renderedContent ? "medium" : "high", title: "The initial HTML is essentially empty", summary: "Search engines that don't run JavaScript see an empty page.", whyItMatters: "Slow connections, link previews, and some crawlers see no useful page content.", exactFix: "Render meaningful page metadata and primary content in the initial response, or configure server rendering for the landing route.", evidence: [requestEvidence({ request: `GET ${ctx.resolvedUrl}`, status: ctx.status })] }));
    }
    const h1s = headings({ html: seoHtml, level: 1 });
    ...
    const missingAlts = imageAlts({ html: seoHtml }).filter((image) => !image.alt);
